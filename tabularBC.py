...
# fileNames = ["coil2000.csv","ozoneOnehr.csv", "mustVersion2.csv","talendHC6.csv"]
# fileNames = ["mustVersion2.csv", 'segmentBine1vAll.csv', 'landsatSatelliteBine1vAll.csv', 'epilepticSeizure1vAll.csv']
# fileNames = ["mustVersion2.csv",'segmentBine1vAll.csv', 'landsatSatelliteBine1vAll.csv', 'epilepticSeizure1vAll.csv']

fileNames = ["aps_failure_all.csv"]

# coil2000 0.05, 0.025, 0.01
# ozoneOnehr 0.025, 0.01
# aps_failure_all.csv 0.01
...
for file in fileNames:
	...
	if minSize > maxSize * 0.01:
		minIdx = np.where(y==minClsIdx)[0]
		if file == "mustVersion2.csv":
			sbsMinIdx = np.random.choice(minIdx, int(maxSize * 0.01),replace=False)
		else:
			sbsMinIdx = np.random.choice(minIdx, int(maxSize * 0.01),replace=False)
	...
	mixNames = []
	for mixUpType, a, b in [['none',0,0],['WeightedBase',0,0],['SMOTE', 0, 0],['basicMix',0.05,0.05],['balanceMix',0.05,0.05],['balanceMix',0.75,5]]:
